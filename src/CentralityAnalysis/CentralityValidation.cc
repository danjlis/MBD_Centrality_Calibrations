#include "CentralityValidation.h"
...
CentralityValidation::CentralityValidation(const std::string &name, const std::string &hist_name)
  : SubsysReco(name)
{
  _hist_filename = hist_name;
}

CentralityValidation::~CentralityValidation()
{
  delete hm;
}
int CentralityValidation::Init(PHCompositeNode * /*unused*/)
{
  // Histograms

  hm = new Fun4AllHistoManager("CENTRALITY_VALIDATION_HIST");
  int nbins = 100;
  _h_centrality_bin = new TH1D("h_centrality_bin", ";Centrality Bin; 1/N{_Events}",nbins+1,-0.005, 1.005);
  _h_centrality_bin_mb = new TH1D("h_centrality_bin_mb", ";Centrality Bin; 1/N{_Events}",nbins+1,-0.005, 1.005);
...
int CentralityValidation::InitRun(PHCompositeNode * /*unused*/)
{
  if (Verbosity())
  {
...
int CentralityValidation::process_event(PHCompositeNode *topNode)
{
  if (Verbosity())
  {
...
int CentralityValidation::End(PHCompositeNode * /* topNode*/)
{
