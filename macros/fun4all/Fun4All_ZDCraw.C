...
void Fun4All_ZDCraw(const int runnumber, const int rollover = 0)
{
  ...
  const char *tree_outfile = Form("/gpfs02/%s/output/run%d/waves/zdc_waveform_tree_%s_%s.root", env_p, runnumber, rstr.str().c_str(), ostr.str().c_str());

  std::string fname1 = Form("/sphenix/lustre01/sphnxpro/commissioning/ZDC/beam/beam_seb14-%s-%s.prdf", rstr.str().c_str(), ostr.str().c_str());
  ...
  CaloPacketGetter *ca = new CaloPacketGetter("CALOPACKETGETTER_ZDC","ZDC");
  ca->set_nsamples(31);
  se->registerSubsystem(ca);

  WaveTreeMaker *tt1 = new WaveTreeMaker("WAVEMAKER_ZDC",tree_outfile);
  //tt1->SetVerbosity(1);
