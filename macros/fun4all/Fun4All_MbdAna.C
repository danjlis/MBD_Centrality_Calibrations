...
const char* env_p = std::getenv("MBD_CENTRALITY_CALIB_PATH");
  ...
  const char *tree_outfile = Form("%s/mbd_calib_ana_tree_%s_%s.root", env_p, rstr.str().c_str(), ostr.str().c_str());
  ...
  MbdAnalysis *cr = new MbdAnalysis("MbdAnalysis",tree_outfile);
