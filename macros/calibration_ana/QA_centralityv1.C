...
float GetZVertex(float *mbd_charge, float *mbd_time, float *zdc_sum);
		   ...
		   )
{
  ...
  QA_MakeChargeSum(runnumber, 1, 0);
  //  QA_MakeCentralityCalibrations(runnumber, 1, false);
...
void QA_MakeChargeSum(const int runnumber, const int loadRunCalibration = 1, const int shift_runnumber = 0)
{
  ...
  if (loadRunCalibration)
      ...
      if (!tcalibfile)
	{
      ...
      for (int i = 0 ; i < ts->GetEntries(); i ++)
 	{
      ...
      tcalibfile = new TFile(Form("%s/calib/slew_calib_mbd_%d.root", env_p, runnumber), "r");
      ...
      if (!tcalibfile)
	{
      ...
      for (int i = 0 ; i < 128; i ++) f_slewing_corrections[i] = (TF1*) tcalibfile->Get(Form("f_slew_channel_%d", i));
  ...
  TH1D *h_dslew_vertex = new TH1D("h_dslew_vertex", "", nbin, -50, 50);
  ...
  TH2D *nhits_ns = new TH2D("nhits_ns",";nhits n;nhits s",65, -0.5, 64.5, 65,-0.5, 64.5);
  TH2D *nhits_ns_no_vtx = new TH2D("nhits_ns_no_vtx",";nhits n;nhits s",65, -0.5, 64.5, 65,-0.5, 64.5);
  TH2D *nhits_ns_vtx_no_min_bias = new TH2D("nhits_ns_vtx_no_min_bias",";nhits n;nhits s",65, -0.5, 64.5, 65,-0.5, 64.5);
  TH2D *nhits_ns_min_bias_vtx = new TH2D("nhits_ns_min_bias_vtx",";nhits n;nhits s",65, -0.5, 64.5, 65,-0.5, 64.5);
  TH2D *nhits_ns_w_gt10_dv = new TH2D("nhits_ns_w_gt10_dv",";nhits n;nhits s",65, -0.5, 64.5, 65,-0.5, 64.5);
  TH2D *nhits_ns_w_lt10_dv = new TH2D("nhits_ns_w_lt10_dv",";nhits n;nhits s",65, -0.5, 64.5, 65,-0.5, 64.5);
  ...
  for (int i = 0 ; i < (DEBUG ? 10 : t->GetEntries()); i++)
      ...
      for (int ich = 0 ; ich < 128; ich++)
	{
	  ...
	  mbd_time[ich] = ((25. - mbd_time_raw[ich] * (9.0 / 5000.) - time_shift_corr[ich]))*time_scale_corr[ich] - f_slewing_corrections[ich]->Eval(mbd_charge_raw[ich]);
	  mbd_time_noslew[ich] = ((25. - mbd_time_raw[ich] * (9.0 / 5000.) - time_shift_corr[ich]))*time_scale_corr[ich];
      ...
      float old_z = GetZVertex(mbd_charge, mbd_time_noslew, zdc_sum);
      sort(time_sum_n.begin(), time_sum_n.end());
      sort(time_sum_s.begin(), time_sum_s.end());

      minbias = (zdc_sum[0] > 40 && zdc_sum[1] > 40);
	...
	{
	  z_vertex = TMath::C()*(1e-7/2.)*(mean_north - mean_south);
      ...
      if (hits_s_t >= central_cut && hits_n_t >= central_cut)
	{
      ...
      if (old_z != 999 && z_vertex != 999)
	{
	  h_dslew_vertex->Fill(old_z - z_vertex);
	  if (old_z - z_vertex > 10) nhits_ns_w_gt10_dv->Fill(hits_n, hits_s);
	  if (fabs(old_z - z_vertex) < 10) nhits_ns_w_lt10_dv->Fill(hits_n, hits_s);
	}
      nhits_ns->Fill(hits_n, hits_s);
      ...
      if (TMath::Abs(z_vertex) > 30) {
	nhits_ns_no_vtx->Fill(hits_n, hits_s);
	continue;
      }  

      h_charge_sum_vtx->Fill(charge_sum);

      if (!minbias) {
	nhits_ns_vtx_no_min_bias->Fill(hits_n, hits_s);
	continue;
      }

      nhits_ns_min_bias_vtx->Fill(hits_n, hits_s);
  ...
  h_vertex->Write();
  nhits_ns->Write();
  nhits_ns_no_vtx->Write();
  nhits_ns_vtx_no_min_bias->Write();
  nhits_ns_min_bias_vtx->Write();
  nhits_ns_w_lt10_dv->Write();
  nhits_ns_w_gt10_dv->Write();
  h_dslew_vertex->Write();
...
void QA_MBDCalibrations(const int runnumber, const int doPerRunCalibration, const int makeNewHistograms)
{
  ...
  TF1 *f_lan_w_gausexp = new TF1("lan_w_gausexp","[0]*TMath::Landau(x,[1],[2],3) + [3]*TMath::Exp(-1*(x - [4])/[5])+gaus(6)", 0, 2000);
  ...
  // TH1D *h_blank = (TH1D*) h_charge[0]->Clone();
  // h_blank->Reset();
  // h_blank->Fill(1);
  // Fitting to check
  // for (int ich = 0 ; ich < 128; ich++){
...
void QA_MBDTimeCalibrations(const int runnumber, const int useChargeTime = 0)
{
  ...
  float binlengtht = 0.01;
  float binlengtht2 = 0.2;
  float binlengthc = 2;

  int nbins_t = floor((high_t - low_t)/binlengtht);
  int nbins_t2 = floor((high_t - low_t)/(binlengtht2));
  int nbins_c = floor((high_c - low_c)/(binlengthc*10.));
  const int nbinedges = 1 + 300/10 + 500/20 + 700/50 + 500/100;
  double bin_edges_c[nbinedges];

  bin_edges_c[0] = 0;
  for (int i = 1; i < nbinedges; i++)
    {
      double change = 10;
      if (bin_edges_c[i-1] >= 1500) change = 100.;
      else if (bin_edges_c[i-1] >= 800) change = 50.;
      else if (bin_edges_c[i-1] >= 300) change = 20.;
      bin_edges_c[i] = bin_edges_c[i-1] + change;
    }
  ...
  for (int i = 0; i < 128; i++)
    {
      ...
      h_peak_tdc_channel[i] = new TH2D(Form("h_peak_tdc_channel_%d", i), "", nbinedges - 1, bin_edges_c, nbins_t2, low_t, high_t);
  ...
  for (int i = 0; i < t2->GetEntries(); i++)
      ...
      for (int ich = 0; ich < 128; ich++)
	{
	  ...
	  if (mbd_charge[ich]*gain_corr[ich] < 0.4) continue;
	  h_peak_tdc_channel[ich]->Fill(mbd_charge[ich] , mbd_time[ich]);
	  if (useChargeTime)
	    {	  
  ...
  for (int i = 0; i < t2->GetEntries(); i++)
      ...
      for (int ich = 0; ich < 128; ich++)
	{
	  ...
	  if (mbd_charge[ich]*gain_corr[ich] < 0.4) continue;
	  double scale = 10.;
	  if (mbd_charge[ich] >= 1500) scale = 100.;
	  else if (mbd_charge[ich] >= 800) scale = 50.;
	  else if (mbd_charge[ich] >= 300) scale = 20.;

	  h_peak_tdc_channel[ich]->Fill(mbd_charge[ich], tdc, 1./scale);	  
	  if (useChargeTime)
	    {
  ...
  TH1D *h_fit_slew[128]{};
  TGraphErrors *g_fit_slew[128]{};
  TH1D *h_fit_slew_chi2[128]{};
  TF1 *f_fit_slew[128]{};
  TH1D *h_nbin_in_fit[128]{};
  for (int ich = 0;  ich < 128; ich++)
    {
      double lowrange= -1;
      TH1D *h_y_peak_tdc = (TH1D*) h_peak_tdc_channel[ich]->ProjectionY()->Clone();

      for (int i = 1; i <= h_y_peak_tdc->GetNbinsX(); i++)
	{
	  if (h_y_peak_tdc->GetBinContent(i) > 0)
	    {
	      lowrange = h_y_peak_tdc->GetBinCenter(i+2);
	      break;
	    }
	}

      if (lowrange < -1) lowrange = -1;

      double highrange = 7.0;
      std::pair<double, double> range = std::make_pair(lowrange, highrange);  

      TF1 *f_slew = new TF1("f_slew", "[0] + [1]/TMath::Sqrt(x) + [2]*TMath::Exp([3]*(x - [4]))", 50, 2000);
      //TF1 *f_slew = new TF1("f_slew", "[0] + [1]*x + [2]*TMath::Exp([3]*([4] + x))", 100, 2000);

      f_slew->SetParameter(0, -0.5);
      f_slew->SetParameter(1, 5);
      f_slew->SetParameter(2, 8);
      f_slew->SetParameter(3, -0.007);
      f_slew->SetParameter(4, -8);
      // f_slew->SetParameter(0, -0.5);
      // f_slew->SetParameter(1, 10);
      // f_slew->SetParameter(2, -0.1);
      // f_slew->SetParameter(3, 2);
      // f_slew->SetParameter(4, -0.1);

//       f_slew->SetParameter(0, 0.08);
//       f_slew->SetParameter(1, -0.0004);
//       f_slew->SetParameter(2, 1);
//             f_slew->SetParameter(3, 23);
//       f_slew->SetParameter(4, 0.011);
//  
      int nparams = 6;
      double params[6]= {10, 0, .5, 5, 5.1, 1};

      std::cout << " Channel "<<ich<<std::endl;
      std::cout << " " << std::endl;
      std::cout << "   Range: "<<range.first<<" --> "<<range.second << std::endl;
      TF1 *f_slice = new TF1("double_gaussian","gaus(0)+gaus(3)",range.first, range.second);

      for (int i = 0 ; i < nparams; i++)
	{
	  f_slice->SetParameter(i, params[i]);
	}

      h_peak_tdc_channel[ich]->FitSlicesY(f_slice, 1, nbins_c, 30, "QR");
      h_fit_slew[ich] = (TH1D*) gDirectory->Get(Form("h_peak_tdc_channel_%d_1", ich));
      h_fit_slew_chi2[ich] = (TH1D*) gDirectory->Get(Form("h_peak_tdc_channel_%d_chi2", ich));

      // setting the 
      h_nbin_in_fit[ich] = new TH1D(Form("h_nbin_in_fit_%d", ich), "", nbinedges, -0.5, nbinedges - 0.5);

      for (int i = 1; i <= h_nbin_in_fit[ich]->GetNbinsX(); i++)
	{
	  double scale_x = 10.;
	  if (bin_edges_c[i-1] >= 1500) scale_x = 100.;
	  if (bin_edges_c[i-1] >= 800) scale_x = 50.;
	  if (bin_edges_c[i-1] >= 300) scale_x = 20.;
	  for (int j = 1; j <= h_peak_tdc_channel[ich]->GetNbinsY(); j++)
	    {
	      if (h_peak_tdc_channel[ich]->GetYaxis()->GetBinCenter(j) > range.second || h_peak_tdc_channel[ich]->GetYaxis()->GetBinCenter(j) < range.first) continue;
	      if (h_peak_tdc_channel[ich]->GetBinContent(i, j) > 0) h_nbin_in_fit[ich]->Fill(i);
	    }
	  h_nbin_in_fit[ich]->SetBinContent(i, h_nbin_in_fit[ich]->GetBinContent(i) - 6);
	}
      ...
      for (int i = 1; i <= h_fit_slew[ich]->GetNbinsX(); i++)
	{
	  if (h_fit_slew_chi2[ich]->GetBinContent(i) > 10 ) continue;
	  if (h_fit_slew_chi2[ich]->GetBinContent(i) > 4 && h_fit_slew_chi2[ich]->GetBinCenter(i) > 1400 ) continue;
	  if (h_fit_slew[ich]->GetBinContent(i) == 0 || h_fit_slew[ich]->GetBinError(i) == 0) continue;
	  if (h_fit_slew[ich]->GetBinError(i) > 0.5 ) continue;
	  if (h_fit_slew[ich]->GetBinError(i) < 0.01 && h_fit_slew[ich]->GetBinCenter(i) > 1000) continue;
	  if (h_fit_slew[ich]->GetBinContent(i) > 2 || h_fit_slew[ich]->GetBinContent(i) < -1) continue;
	  x.push_back(h_fit_slew[ich]->GetBinCenter(i));
	  x_err.push_back((h_fit_slew[ich]->GetBinCenter(i)-h_fit_slew[ich]->GetBinLowEdge(i)));
	  y.push_back(h_fit_slew[ich]->GetBinContent(i));
	  y_err.push_back(h_fit_slew[ich]->GetBinError(i));
	}
      ...
      for (int i = 0; i < size; i++)
	{
      ...
      g_fit_slew[ich] = new TGraphErrors(size, xx, yy, xxee, yyee);
      g_fit_slew[ich]->SetName(Form("g_fit_slew_%d", ich));

      double lowbar = 40;      
      double highbar = xx[size - 1];

      for (int i = 2 ; i <= h_fit_slew[ich]->GetNbinsX(); i++)
	{
	  if (h_fit_slew[ich]->GetBinContent(i))
	    {
	      lowbar = h_fit_slew[ich]->GetBinCenter(i+1);
	      if (lowbar < 20) lowbar = 20;
	      break;
	    }
	}
      std::cout << "   Range: "<<lowbar<<" --> "<<highbar<< std::endl;
      //f_slew->SetParameter(2, lowbar-2);
      //f_slew->FixParameter(2, lowbar-2);
      auto status = g_fit_slew[ich]->Fit("f_slew","", "");
      TString statusstring = (TString) gMinuit->fCstatu;
      int trys = 0;
      std::cout << trys << " : "<<statusstring <<std::endl;
      while (statusstring.Contains("FAILED") && trys < 5)
	{
	  trys++;
	  f_slew->SetParameter(0, -0.5);
	  f_slew->SetParameter(1, 5);
	  ...
	  // f_slew->SetParameter(0, 0.6);
	  // //      f_slew->SetParameter(1, -0.004);
	  // f_slew->SetParameter(1, -0.05);
	  // f_slew->SetParameter(3, -0.0005);
	  
	  // f_slew->SetParLimits(0, 0,2);
	  // f_slew->SetParLimits(1, -2,0);
	  // f_slew->SetParLimits(3, -2, 0);

	  // f_slew->SetRange(50, 1200 - trys*100);
	  // f_slew->SetParameter(0, 0.08);
	  // f_slew->SetParameter(1, -0.0004);
	  // f_slew->SetParameter(2, 1);
	  // f_slew->SetParameter(3, 23);
	  ...
	  // f_slew->SetParLimits(0, -1, 1);
	  // f_slew->SetParLimits(1, -1, 0);
	  // f_slew->SetParLimits(4, 0, 0.5);

	  // f_slew->SetParameter(0, 0.7);
	  // f_slew->SetParameter(1, -0.0005);
	  // f_slew->SetParameter(2, 60);
	  // f_slew->SetParameter(3, -0.01);
	  // f_slew->SetParameter(4, 400);
	  
	  status = g_fit_slew[ich]->Fit("f_slew","", "", 50, 2000 - trys*100);
	  statusstring = (TString) gMinuit->fCstatu;
	  std::cout << trys << " : "<<statusstring <<std::endl;
	} 
      f_fit_slew[ich] = (TF1*) f_slew->Clone();
      f_fit_slew[ich]->SetName(Form("f_slew_channel_%d", ich));
    }
  ...
  for (int i = 0; i < 128; i++)
    {
      ...
      h_peak_tdc_channel[i]->Write();
      h_fit_slew[i]->Write();
      h_fit_slew_chi2[i]->Write();
      g_fit_slew[i]->Write();
      h_nbin_in_fit[i]->Write();
  ...
  for (int i = 0; i < t2->GetEntries(); i++)
      ...
      for (int ich = 0; ich < 128; ich++)
	{
	  ...
	  double tdc = scale_fit_tdc[ich]*((25. - static_cast<double>(mbd_time[ich])*(9./5000.)) - middle_fit_tdc[ich]) - f_fit_slew[ich]->Eval(mbd_charge[ich]);
  ...
  ftcalib = new TFile(Form("%s/calib/slew_calib_mbd_%d.root", env_p, runnumber),"recreate");
  for (int i = 0; i < 128; i++)f_fit_slew[i]->Write();
  ftcalib->Close();

  TFile *fout3 = new TFile(Form("%s/output/plots/time_calibout_%d.root", env_p, runnumber),"recreate");
  for (int i = 0; i < 128; i++)
    {
      h_tdc_channel[i]->Write();
      h_peak_tdc_channel[i]->Write();
...
void QA_MakeCentralityCalibrations(const int runnumber, const bool doVertexSelection = 1, const bool use_shifted = false)
{
  ...
  sprintf(fdataname,"%s/output/plots/mbd_charge_sum_%d.root", env_p, runnumber);  
  ...
  for (int i=ihigh;i>=1;i--) {
    ...
    if (maxcentbins==9 || maxcentbins == 8) {
      if ((hRealBBC->Integral(i,nhistbins)/hRealBBC->Integral()) > ((cent_bin + 1)*10./forcetrigfrac) && centrality_got[cent_bin] == 0) {
	centrality_low[cent_bin] = hRealBBC->GetBinCenter(i);
	centrality_high[cent_bin+1] = hRealBBC->GetBinCenter(i);
	centrality_got[cent_bin] = 1;
	cent_bin++;
      }
    }
    if (maxcentbins ==19) 
      {
	if ((hRealBBC->Integral(i,nhistbins)/hRealBBC->Integral()) > ((cent_bin + 1)*5./forcetrigfrac) && centrality_got[cent_bin] == 0) {
	  centrality_low[cent_bin] = hRealBBC->GetBinCenter(i);
	  centrality_high[cent_bin+1] = hRealBBC->GetBinCenter(i);
	  centrality_got[cent_bin] = 1;
	  cent_bin++;
